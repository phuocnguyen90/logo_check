...
class TrademarkDataset(Dataset):
    ...
    def __init__(
        ...
        use_instance_discrimination: bool = True  # NEW: Default to Instance-wise SSL
    ):
        ...
        # Only needed if NOT using instance discrimination
        ...
        if not self.use_instance_discrimination:
            ...
            logger.info(f"Initialized TrademarkDataset with {len(metadata)} images (Weak Supervision Mode).")
            logger.info(f"Found {len(self.active_codes)} Vienna codes suitable for positive sampling.")
        else:
            logger.info(f"Initialized TrademarkDataset with {len(metadata)} images (Instance Discrimination Mode).")
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        View 2: 
            - If instance_discrimination=True: Another augmented view of the SAME image (Self-Supervised).
            - If instance_discrimination=False: Another image sharing at least one Vienna code (Weakly Supervised).
        """
        ...
        # Fallback to SSL (same image) if:
        # - Instance discrimination is ON
        # - Or Weak Supervision finding failed (no shared codes or load error)
        if img2 is None:
            img2 = img1.copy()
